...
useEffect(() => {
  if (showLetter || playingVideo) {
    scrollPosition.current = window.scrollY
    document.body.style.overflow = "hidden"
  } else {
    document.body.style.overflow = "auto"

    // Delay scroll restore slightly to allow DOM render
    setTimeout(() => {
      window.scrollTo({
  ...
  // Play video function - FIXED with modal popup
  ...
  // Close video player - FIXED
  const closeVideo = () => {
    if (playingVideo && videoRefs.current[playingVideo]) {
      videoRefs.current[playingVideo].pause()
      videoRefs.current[playingVideo].currentTime = 0; // Reset video to start
    }
    setPlayingVideo(null)
    setVideoLoading(false)
  }

  // Handle video loaded - FIXED
  ...
  // Handle video error - FIXED with fallback
  const handleVideoError = (videoId) => {
    setVideoLoading(false)
    setSecretMessage("Video not found. Using placeholder... 🎥")
    setTimeout(() => setSecretMessage(""), 3000)
    
    // Use fallback video URLs if local videos don't exist
    const video = videoRefs.current[videoId]
    if (video) {
      // You can set placeholder video URLs here
            ...
            <div className="relative bg-gradient-to-br from-pink-900/30 to-red-900/30 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-white/20">
              ...
              <div className="text-center mb-6">
                <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-pink-500 to-red-500 rounded-full mb-4 shadow-lg">
                  <span className="text-3xl">{memories.find(m => m.id === playingVideo)?.emoji}</span>
                </div>
                <h2 className="text-3xl md:text-4xl font-dancing font-bold text-white mb-3">
                  {memories.find(m => m.id === playingVideo)?.title}
                </h2>
                <p className="text-pink-200 text-lg max-w-2xl mx-auto">
                  {memories.find(m => m.id === playingVideo)?.description}
                </p>
              </div>
              ...
              <div className="relative bg-black rounded-2xl overflow-hidden shadow-2xl aspect-video">
                <video
                  ...
                  muted={false}
                  playsInline
                  onLoadedData={() => handleVideoLoaded(playingVideo)}
                  onError={() => handleVideoError(playingVideo)}
                >
                  <source 
                    src={memories.find(m => m.id === playingVideo)?.videoSrc} 
                    type="video/mp4" 
                  />
              ...
              <div className="max-w-4xl mx-auto text-center">
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-3xl p-10 shadow-2xl border border-white/50 mb-10">
                  ...
                  <div className="flex flex-col items-center space-y-6">
                    <button
                      onClick={toggleMusic}
                      className={`px-8 py-3 rounded-full font-semibold shadow-lg transform hover:scale-105 transition-all duration-300 flex items-center space-x-3 ${
                        musicPlaying 
